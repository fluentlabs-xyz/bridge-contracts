...
describe("Contract deployment and interaction", function () {
    ...
    before(async () => {
        ...
        [l1RestakerGatewayContract, l1RestakerFactoryContract, l1RestakerImplementationContract] = await SetUpL1Restaker(l1BridgeContract.target)
    ...
    async function SetUpL2Restaker(bridgeAddress) {
        ...
        let setRatioFeedTx = await protocolConfigContract.setRatioFeed(ratioFeedContract.target)
        ...
        let setLiquidityTokenTx = await protocolConfigContract.setLiquidityToken(liquidityTokenContract.target)
        ...
        let setRestakingPoolTx = await protocolConfigContract.setRestakingPool(restakingPoolContract.target)
        ...
        erc20PeggedTokenContract = await erc20PeggedTokenContract.waitForDeployment();
        ...
        erc20TokenFactoryContract = await erc20TokenFactoryContract.waitForDeployment();
        ...
        upgradeableBeaconContract = await upgradeableBeaconContract.waitForDeployment();
        ...
        eigenPodManagerMockContract = await eigenPodManagerMockContract.waitForDeployment();
        ...
        delegationManagerMockContract = await delegationManagerMockContract.waitForDeployment();
        ...
        restakerDeployerContract = await restakerDeployerContract.waitForDeployment();
        log("restakerDeployerContract.address:", restakerDeployerContract.target);

        let setRestakerDeployerTx = await protocolConfigContract.setRestakerDeployer(restakerDeployerContract.target)
        await setRestakerDeployerTx.wait()

        const transferOwnershipTx = await erc20TokenFactoryContract.transferOwnership(restakerGatewayContract.target);
    ...
    async function SetUpChain(ctx, withRollup) {
        ...
        log("erc20TokenFactoryContract.owner:", await erc20TokenFactoryContract.owner());
        await sleep(1000)
        const transferOwnershipTx = await erc20TokenFactoryContract.transferOwnership(erc20GatewayContract.target);
        await transferOwnershipTx.wait();
        log("erc20TokenFactoryContract.owner:", await erc20TokenFactoryContract.owner());
        ...
        return [erc20GatewayContract, bridgeContract, erc20PeggedTokenContract.target, erc20TokenFactoryContract.target];
    }

    it("Compare pegged token addresses", async function () {
        let t1 = await l2GatewayContract.computePeggedTokenAddress(l1TokenContract.target);
        let t2 = await l1GatewayContract.computeOtherSidePeggedTokenAddress(l1TokenContract.target);
    ...
    it("Bridging tokens between to contracts", async function () {
        ...
        const approveTx = await l1TokenContract.approve(l2GatewayContract.target, 100);
        await approveTx.wait();

        log("Token send");

        let liquidityTokenAmount = await liquidityTokenContract.convertToAmount(1);
        log("liquidityTokenContract.address:", liquidityTokenContract.target, "liquidityTokenAmount:", liquidityTokenAmount)
        for (let v of [ctxL1, ctxL2]) {
            await v.printDebugInfoAsync()
        }
        ...
        log("l1GatewayContract.signer.address:", await l1GatewayContract.signer.getAddress())
