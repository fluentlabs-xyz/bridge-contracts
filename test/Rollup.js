...
describe("Rollup.sol", function () {
  ...
  it("Accept proof", async function () {
    ...
    expect(await rollupContractWithSigner.acceptedBatch(batchIndex)).to.eq(
      false,
    );
    expect(await rollupContractWithSigner.approvedBatch(batchIndex)).to.eq(
      false,
    );
  ...
  it("Proof check", async function () {
    ...
    await rollupContractWithSigner.challengeBatch(nextBatchIndex, {
      value: 10000,
    });
    ...
    await rollupContractWithSigner.proofBatch(
      nextBatchIndex,
