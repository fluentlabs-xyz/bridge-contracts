...
async function main() {
    ...
    console.log("Balance:", await providerL1.getBalance(await signerL1.getAddress()))

    const RollupContract = await ethers.getContractFactory("Rollup");
    let rollup = await RollupContract.connect(signerL1).attach("0xb592Ed460f5Ab1b2eF874bE5e3d0FbE6950127Da");

    const BridgeContract = await ethers.getContractFactory("Bridge");
    let bridge = await BridgeContract.connect(signerL1).attach("0xf70f7cADD71591e96BD696716A4A2bA6286c82e8");
    ...
    let tokenFactory = await TokenFactoryContract.connect(signer).attach("0x43E9dbA5b512774D6Baf41a2c64DD8e4dcff0970")

    console.log("token factory owner: ", await tokenFactory.owner());

    nonce = await signer.getTransactionCount();

    const authTx = await tokenFactory.transferOwnership("0x43E9dbA5b512774D6Baf41a2c64DD8e4dcff0970",
        {
            nonce: 11,
            // gasLimit: 100000,
    ...
    // console.log("TX: ", tx)
    // await tx.wait();
    let tx = await ERC20GatewayContract.connect(signerL2).attach(l2Gateway).setOtherSide(
        l1Gateway,
        l1Implementation,
        l1Factory,
    );
